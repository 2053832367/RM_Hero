...
*/
float number[10];
void autoSolveTrajectory(Visual_New_Receive_Data_t *New_Visusal,Visual_Caculate_Data* AIM_data,fp32 *Gimbal_Yaw)
{
...
//				float dis_OUTPOST_min = sqrt(tar_position[0].x * tar_position[0].x + tar_position[0].y * tar_position[0].y);
//				idx = 0;
//				for (i = 0; i<3; i++)
//				 {
//						float temp_dis_OUTPOST = sqrt(tar_position[i].x * tar_position[0].x + tar_position[i].y * tar_position[0].y);
//						if (temp_dis_OUTPOST < dis_OUTPOST_min)
//						{
//							dis_OUTPOST_min = temp_dis_OUTPOST;
//							idx = i;
//						}
//					}	
...
//		}
	...
	// 自瞄使用的pitch和yaw
		if(Gimbal.Flags.FC_Flag!=true){
		  temp_yaw= float(atan2(caculate_y,caculate_x))*RAD_TO_ANGLE - *Gimbal_Yaw;
					if(temp_yaw>= 180) temp_yaw-=360;		
					if(temp_yaw<=-180) temp_yaw+=360;
			temp_pitch=((pitchTrajectoryCompensation(sqrt(caculate_x * caculate_x +caculate_y * caculate_y),
                     caculate_z - st.z_bias, st.current_v))*RAD_TO_ANGLE - AIM_data->Visual_Caculate_Pitch);
			
			
				New_Visusal->New_Visual_Yaw 	= temp_yaw + New_Visusal->static_yaw;
				New_Visusal->New_Visual_Pitch = temp_pitch + New_Visusal->static_pitch;
		}
		if(Gimbal.Flags.FC_Flag){
		// 火控使用的pitch和yaw误差值
			New_Visusal->New_Visual_Yaw   = float(atan2(New_Visusal->y, New_Visusal->x))*RAD_TO_ANGLE - *Gimbal_Yaw;
			New_Visusal->New_Visual_Pitch = (pitchTrajectoryCompensation(sqrt(New_Visusal->x * New_Visusal->x +New_Visusal->y * New_Visusal->y),
						  	                       New_Visusal->z - st.z_bias, st.current_v))*RAD_TO_ANGLE - AIM_data->Visual_Caculate_Pitch+ New_Visusal->static_pitch;
		
		number[0]=(pitchTrajectoryCompensation(sqrt(New_Visusal->x * New_Visusal->x +New_Visusal->y * New_Visusal->y),
						  	                       New_Visusal->z - st.z_bias, st.current_v))*RAD_TO_ANGLE;
		}
		
		f1.y_error=New_Visusal->y - caculate_y;
		f1.time_error=(f1.y_error/f1.vter) - 0.8333f + timeDelay;
//	f1.time_error=(f1.y_error/f1.vter) + timeDelay;

		number[1]=caculate_y;number[2]=New_Visusal->y;
	if( Gimbal.Mode==GIMBAL_AIM && Gimbal.Flags.FC_Flag)
	{
		if(Gimbal.RC.read_key(&Gimbal.RC.Key.Z,single,true))
		{
				Flaggg=1;
		}
			if(f1.time_error > -0.03f && f1.time_error < 0     && Message.New_VisualR.tracking==1  && Gimbal.Data.Now_shooter_barrel_heat<50 && Flaggg)
			{ 
				 f1.fire_tracking=1;
			}
			else f1.fire_tracking = 0;
	}
	else f1.fire_tracking = 0;
