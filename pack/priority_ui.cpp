...
#define HIGH_CHAR_PRIORITY_LEVEL 8  // 高优先级字符抢占图形UI的等级
#define MID_CHAR_PRIORITY_LEVEL  5  // 中优先级字符抢占图形UI的等级

#define SEND_INTERVAL        200 // 发送间隔时间(MS) [裁判系统上限是10HZ]
...
ext_client_custom_character_t Process_Char_Info_To_Buffer(ui_info_t ui_info, uint8_t add_operate_enable)
{
  ...
  memcpy((void*)&res.data,(const void*)&ui_info.ui_config.text,sizeof(ui_info.ui_config.text));
	
//memcpy(&Graph_type.remote_control, (&Rx_Message[start_pos] + 7), Graphdatahead.DataLength);
...
void Ui_Send()
{
	send_test=  Gimbal.Flags.send_test_Flag;
  /*判断是否到了发送时间****************************/
  uint32_t currentTick = HAL_GetTick();
  ...
  /*收到初始化指令切换标志位**********************/
  //static uint8_t is_initing_ui = 1;
...
//  }
  ...
  if ( Gimbal.Flags.send_test_Flag)//正在初始化UI
  {
		Ui_Send_Add();
//    if (Ui_Send_Add() == 1)
//    {
