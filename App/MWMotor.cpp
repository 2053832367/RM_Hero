...
void MotorSet(uint8_t busId, uint8_t can_id, uint8_t *data, uint8_t dataSize) 
{
  if(busId == 0x001) 
	  CAN_Cmd.SendData(&hfdcan1,can_id,data,8);
...
void MWHeartbeatRec(MW_MOTOR_DATA *motorData, uint8_t *data) {
    memcpy(&(motorData->axisError), &data[0], sizeof(motorData->axisError));
	  motorData->Axis_State=data[4];
		motorData->Error_Flags.motorErrorFlag     =data[5]&0x01;
		motorData->Error_Flags.encoderErrorFlag   =data[5]&0x02;
		motorData->Error_Flags.controllerErrorFlag=data[5]&0x03;
	  motorData->Error_Flags.systemErrorFlag    =data[5]&0x04;
...
void MWGetMotorErrorRec(MW_MOTOR_DATA *motorData, uint8_t *data) {
    motorData->Error_Flags.motorErrorFlag     =data[0]&0x01;
		motorData->Error_Flags.encoderErrorFlag   =data[0]&0x02;
		motorData->Error_Flags.controllerErrorFlag=data[0]&0x03;
	  motorData->Error_Flags.systemErrorFlag    =data[0]&0x04;
}
