...
void RC_Init(uint8_t *rx1_buf, uint8_t *rx2_buf, uint16_t dma_buf_num)
{
    ...
    while(((DMA_Stream_TypeDef*) hdma_usart2_rx.Instance)->CR & DMA_SxCR_EN)
    {
        __HAL_DMA_DISABLE(&hdma_usart2_rx);
    }

    ((DMA_Stream_TypeDef*) hdma_usart2_rx.Instance)->PAR = (uint32_t) & (USART2->RDR);
    //memory buffer 1
    //内存缓冲区1
    ((DMA_Stream_TypeDef*) hdma_usart2_rx.Instance)->M0AR = (uint32_t)(rx1_buf);
    //memory buffer 2
    //内存缓冲区2
    ((DMA_Stream_TypeDef*) hdma_usart2_rx.Instance)->M1AR = (uint32_t)(rx2_buf);
    //data length
    //数据长度
    ((DMA_Stream_TypeDef*) hdma_usart2_rx.Instance)->NDTR = dma_buf_num;
    //enable double memory buffer
    //使能双缓冲区
    SET_BIT(((DMA_Stream_TypeDef*) hdma_usart2_rx.Instance)->CR, DMA_SxCR_DBM);
